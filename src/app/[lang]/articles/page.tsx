...
export default async function Page({ params }: { params: Promise<{ lang: string }> }) {
  ...
  // Pre-render articles per category per page
  const categoryIds = CATEGORIES.map(c => c.id);
  const pagesByCat: Record<string, React.ReactNode[]> = {};

  for (const catId of categoryIds) {
    const catArticles = catId === 'All' ? allArticles : allArticles.filter((a: any) => a.category === catId);
    const totalPages = Math.ceil(catArticles.length / ARTICLES_PER_PAGE) || 1;
    const pageNodes: React.ReactNode[] = [];

    for (let page = 1; page <= totalPages; page++) {
      const pageArticles = catArticles.slice((page - 1) * ARTICLES_PER_PAGE, page * ARTICLES_PER_PAGE);
      pageNodes.push(
        <div
          key={`${catId}-${page}`}
          className={`article-page${catId === 'All' && page === 1 ? '' : ' hidden'}`}
          data-cat={catId}
          data-page={page}
        >
    ...
    pagesByCat[catId] = pageNodes;
  }

  // Compute total pages per category
  const totalPagesByCat: Record<string, number> = {};
  for (const catId of categoryIds) {
    const catArticles = catId === 'All' ? allArticles : allArticles.filter((a: any) => a.category === catId);
    totalPagesByCat[catId] = Math.ceil(catArticles.length / ARTICLES_PER_PAGE) || 1;
  }
      ...
      <section className="max-w-[1400px] mx-auto px-6 py-16">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <ArticleListClient
            ...
            totalPagesByCat={totalPagesByCat}
          >
            {Object.values(pagesByCat).flat()}
          </ArticleListClient>
        </div>
      </section>

      {/* Inline script for category filtering */}
