...
{/* Hero Section */}
      <div className="relative pt-24 pb-16 md:pt-32 md:pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="mb-10 pb-10 border-b border-white/10 flex flex-wrap items-center gap-4 md:gap-5">
            ...
            <div className="flex flex-wrap items-center gap-3 ml-1 md:ml-3">
                {(city.tags || []).map((tag: any, idx: number) => {
                  ...
                  return (
                    <span key={idx} className="px-4 py-1.5 bg-white/15 backdrop-blur-sm text-white text-[11px] font-bold rounded-full border border-white/20 tracking-wider uppercase shadow-sm">
                      {tagText}
                    </span>
                  );
                })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
            ...
            {/* Weather / Info Card */}
            <div className="col-span-1 bg-white/10 backdrop-blur-xl rounded-2xl p-6 text-white shadow-2xl border border-white/15 transition-all duration-300 hover:bg-white/15 hover:border-white/25">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-white/15 pt-5">
                <div className="text-center p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                  ...
                  <div className="text-xs text-white/50 mb-1">{t('city.weather.area')}</div>
                  <div className="font-bold text-lg text-white">{city.translations?.[language]?.info?.area || city.info?.area || '-'}</div>
                </div>
                <div className="text-center p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                  <Users className="w-5 h-5 mx-auto text-white/50 mb-2" />
                  <div className="text-xs text-white/50 mb-1">{t('city.weather.population')}</div>
                  <div className="font-bold text-lg text-white">{city.translations?.[language]?.info?.population || city.info?.population || '-'}</div>
                </div>
              </div>
            </div>
            ...
            {city.videoUrl && (
              <div className="col-span-1 bg-white/10 backdrop-blur-xl rounded-2xl overflow-hidden text-white shadow-2xl border border-white/15 cursor-pointer group transition-all duration-300 hover:bg-white/15 hover:border-white/25 hover:shadow-3xl" onClick={() => setShowVideoModal(true)}>
      ...
      <div className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            {/* Travel Time */}
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-8 tracking-tight text-center md:text-left flex items-center gap-3 justify-center md:justify-start">
                  <span className="w-1 h-8 bg-gradient-to-b from-[#1b887a] to-emerald-400 rounded-full"></span>
                  {getCityName()}{t('city.bestTime.title')}
                </h2>
                <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-8 shadow-sm border border-gray-100 relative transition-all duration-300 hover:shadow-md hover:border-[#1b887a]/20">
                  <div className="absolute top-0 left-8 -translate-y-1/2 text-6xl text-[#1b887a]/10 font-serif leading-none">"</div>
                  <p className="text-lg text-gray-700 leading-relaxed font-medium mb-4 relative z-10">
                    ...
                    <strong className="text-[#1b887a] font-bold ml-1">
                  ...
                  <div className="absolute bottom-0 right-8 translate-y-1/2 text-6xl text-[#1b887a]/10 font-serif leading-none rotate-180">"</div>
                </div>
              </div>
            ...
            {/* History */}
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-8 tracking-tight text-center md:text-left flex items-center gap-3 justify-center md:justify-start">
                  <span className="w-1 h-8 bg-gradient-to-b from-amber-500 to-orange-400 rounded-full"></span>
                  {getCityName()}{t('city.history.title')}
                </h2>
                <div className="relative border-l-2 border-amber-200 pl-8 space-y-8 pb-4">
                  {getI18nArrayItems('history').map((item: any, idx: number) => (
                    <div key={idx} className="relative group">
                      <div className="absolute -left-[41px] bg-gradient-to-br from-amber-500 to-orange-400 w-4 h-4 rounded-full border-4 border-white shadow-sm group-hover:scale-150 transition-transform duration-300"></div>
                      <div className="bg-gradient-to-br from-white to-amber-50/50 rounded-xl p-5 border border-transparent hover:border-amber-200/50 transition-all duration-300 hover:shadow-sm">
                        <div className="text-sm font-bold text-amber-600 mb-1">{item.year || item.enYear || ''}</div>
      ...
      <div className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-3 tracking-tight flex items-center justify-center gap-3">
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-[#1b887a] to-transparent hidden sm:block"></span>
              {getCityName()}{t('city.attractions.title')}
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-[#1b887a] to-transparent hidden sm:block"></span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {getI18nArrayItems('attractions').map((spot: any, idx: number) => (
              <div key={idx} className="bg-white border text-left border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col hover:-translate-y-1 hover:border-[#1b887a]/20">
                <div className="h-48 bg-gray-100 relative overflow-hidden">
                  <img src={spot.imageUrl || `https://images.unsplash.com/photo-1540202403-b712e0e026ee?w=600&q=80&auto=format&fit=crop&random=${idx}`} alt={spot.name} className="w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110" />
                  {/* A-level Badge */}
                  {spot.level && (
                    <div className="absolute top-2 right-2 bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-2 py-0.5 text-[10px] font-bold rounded flex items-center gap-0.5 shadow-lg backdrop-blur-sm">
                      <Star className="w-3 h-3 fill-current" />
                      <span>国家{spot.level === '5A' ? 'AAAAA' : spot.level === '4A' ? 'AAAA' : spot.level === '3A' ? 'AAA' : spot.level}级</span>
                    </div>
                  )}
                  {spot.rating && !spot.level && (
                    <div className="absolute top-3 right-3 bg-gradient-to-r from-amber-400 to-amber-500 text-white text-xs font-bold px-2.5 py-1 rounded-full flex items-center gap-1 shadow-lg">
                      <Star className="w-3 h-3 fill-current" />
                        {spot.rating}
                    </div>
                  )}
                ...
                <div className="p-5 flex-grow flex flex-col">
                  ...
                  <p className="text-base text-gray-600 mb-5 line-clamp-2 leading-relaxed flex-grow">{spot.desc || spot.enDesc || ''}</p>
        ...
        <div className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-6 text-center">
            <div className="mb-12">
              <h2 className="text-3xl font-bold text-gray-900 mb-3 tracking-tight flex items-center justify-center gap-3">
                <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-amber-600 to-transparent hidden sm:block"></span>
                {getCityName()}{t('city.heritage.world.title')}
                <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-amber-600 to-transparent hidden sm:block"></span>
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {getI18nArrayItems('worldHeritage').length > 0 ? getI18nArrayItems('worldHeritage').map((heritage: any, idx: number) => (
                <div key={idx} className="relative h-64 rounded-xl overflow-hidden group cursor-pointer text-left shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-1">
                  <img src={heritage.imageUrl || `https://images.unsplash.com/photo-1599571234909-29ed5d1321d6?w=600&q=80&auto=format&fit=crop&random=${idx}`} alt={heritage.name} className="w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent"></div>
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-gray-900 text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">
                    {heritage.year || heritage.enYear || ''}
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    <h3 className="text-xl font-bold text-white mb-2 drop-shadow-sm">{heritage.name || heritage.enName || ''}</h3>
                    <p className="text-white/80 text-xs line-clamp-2 leading-relaxed opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">{heritage.desc || heritage.enDesc || ''}</p>
                  </div>
                </div>
              )) : (
       ...
       {/* Optional Intangible Cultural Heritage */}
        <div className="py-16 bg-white">
          <div className="max-w-5xl mx-auto px-6 text-center">
            <div className="mb-12">
              <h2 className="text-3xl font-bold text-gray-900 mb-3 tracking-tight flex items-center justify-center gap-3">
                <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-purple-500 to-transparent hidden sm:block"></span>
                {getCityName()}{t('city.heritage.intangible.title')}
                <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-purple-500 to-transparent hidden sm:block"></span>
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              {getI18nArrayItems('intangibleHeritage').length > 0 ? getI18nArrayItems('intangibleHeritage').map((item: any, idx: number) => (
                <div key={idx} className={`${idx === 2 && getI18nArrayItems('intangibleHeritage').length === 3 ? 'md:col-span-2' : ''} bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 hover:border-purple-200/50 group`}>
                  <div className={`w-full ${idx === 2 && getI18nArrayItems('intangibleHeritage').length === 3 ? 'md:w-1/4' : 'md:w-1/3'} flex-shrink-0 h-40 relative overflow-hidden rounded-xl`}>
                    <img src={item.imageUrl || `https://images.unsplash.com/photo-1544025162-811c75c82de1?w=400&q=80&auto=format&fit=crop&random=${idx}`} alt={item.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105 rounded-lg" />
                  </div>
      ...
      <div className="py-16 bg-gray-50 text-center">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-3 tracking-tight flex items-center justify-center gap-3">
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-blue-500 to-transparent hidden sm:block"></span>
              {getCityName()}{t('city.transport.title')}
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-blue-500 to-transparent hidden sm:block"></span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
            {getI18nArrayItems('transportation').map((item: any, idx: number) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 hover:border-blue-200/50">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="p-3 bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-xl shadow-sm">
      ...
      <div className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <div className="mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-3 tracking-tight flex items-center justify-center gap-3">
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-orange-500 to-transparent hidden sm:block"></span>
              {getCityName()}{t('city.food.title')}
              <span className="w-8 h-0.5 bg-gradient-to-r from-transparent via-orange-500 to-transparent hidden sm:block"></span>
            </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left">
            {getI18nArrayItems('food').map((food: any, idx: number) => (
              <div key={idx} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex gap-5 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 hover:border-orange-200/50 group">
                <div className="w-32 h-32 flex-shrink-0 relative overflow-hidden rounded-xl">
                  <img src={food.imageUrl || `https://images.unsplash.com/photo-1544025162-811c75c82de1?w=400&q=80&auto=format&fit=crop&random=${food.imageIdx || idx}`} alt={food.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                </div>
                <div className="flex-grow flex flex-col justify-center">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="text-green-600 font-semibold bg-green-50 px-2.5 py-0.5 rounded-full text-sm">{food.price}</span>
                  </div>
                  <p className="text-sm text-gray-600 leading-relaxed mb-3 line-clamp-2">{food.desc || food.enDesc || ''}</p>
                  <div className="text-xs text-gray-500 mt-auto w-full border-t border-gray-100 pt-2">
      ...
      <div className="py-16 bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-8 tracking-tight flex items-center gap-3">
            <span className="w-1 h-6 bg-gradient-to-b from-[#1b887a] to-emerald-400 rounded-full"></span>
            {t('city.explore.title')}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { id: 'travel', title: t('city.explore.travel'), icon: '✈️', desc: 'Transportation, visa, and travel tips' },
              { id: 'hotels', title: t('city.explore.hotels'), icon: '🏨', desc: 'Recommended places to stay in ' + getCityName() },
              { id: 'tools', title: t('city.explore.tools'), icon: '🛠️', desc: 'Name generators, translators, and more' }
            ].map((item) => (
              <button key={item.id} className="group flex flex-col items-start p-5 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-[#1b887a]/20 text-left cursor-pointer">
                <span className="text-2xl mb-3">{item.icon}</span>
                <span className="text-gray-900 font-bold mb-1.5 group-hover:text-[#1b887a] transition-colors">{item.title}</span>
                <span className="text-xs text-gray-400 mb-3">{item.desc}</span>
                <span className="flex items-center gap-1 text-xs font-medium text-[#1b887a] opacity-0 group-hover:opacity-100 transition-opacity">
                  {isEn ? 'Learn more' : '了解更多'} <ArrowRight className="w-3 h-3" />
                </span>
              </button>
            ))}
          </div>
